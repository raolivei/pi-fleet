...
Default hostname will be `raspberrypi`. We'll change it to `node-1.eldertree.local` during setup.
...
# This will:
# - Set hostname to 'node-1.eldertree.local'
...
# - pi_host: "192.168.2.86" (or node-1.eldertree.local if DNS configured)
# - pi_user: "raolivei"
