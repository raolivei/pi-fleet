...
echo "  kubectl exec -n vault vault-0 -- sh -c \"VAULT_TOKEN='YOUR_ROOT_TOKEN' vault kv put secret/pi-fleet/cloudflare-tunnel/token token='YOUR_TOKEN_HERE'\""
echo ""
...
echo "    kubectl exec -n vault vault-0 -- sh -c \"VAULT_TOKEN='\${VAULT_TOKEN}' vault kv put secret/pi-fleet/cloudflare-tunnel/token token='YOUR_TOKEN_HERE'\""
echo "  fi"
echo ""
