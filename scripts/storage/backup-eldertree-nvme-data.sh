...
DATA_SIZE=$(sshpass -p "$PI_PASSWORD" ssh -o StrictHostKeyChecking=no raolivei@$HOST "sudo du -sh /mnt/nvme 2>/dev/null | awk '{print \$1}'" 2>&1)
...
if [ -z "$BACKUP_DEST" ]; then
    ...
    BACKUP_DEV=$(sshpass -p "$PI_PASSWORD" ssh -o StrictHostKeyChecking=no raolivei@$HOST "lsblk -o NAME,SIZE,TYPE | grep -E 'sd[a-z]|nvme' | grep -v 'nvme0n1' | head -1 | awk '{print \$1}'" 2>&1)
    
    if [ -n "$BACKUP_DEV" ]; then
        echo "Found potential backup device: /dev/$BACKUP_DEV"
        # Check if it's mounted
        MOUNT_POINT=$(sshpass -p "$PI_PASSWORD" ssh -o StrictHostKeyChecking=no raolivei@$HOST "mount | grep /dev/$BACKUP_DEV | awk '{print \$3}'" 2>&1)
...
AVAILABLE=$(sshpass -p "$PI_PASSWORD" ssh -o StrictHostKeyChecking=no raolivei@$HOST "df -h $BACKUP_DEST 2>/dev/null | tail -1 | awk '{print \$4}' || echo 'Destination not accessible'" 2>&1)
...
echo "Data to backup:"
sshpass -p "$PI_PASSWORD" ssh -o StrictHostKeyChecking=no raolivei@$HOST "sudo du -sh /mnt/nvme/* 2>/dev/null | sort -h" 2>&1
echo ""
...
sshpass -p "$PI_PASSWORD" ssh -o StrictHostKeyChecking=no raolivei@$HOST "sudo mkdir -p $BACKUP_DEST" 2>&1

# Backup data
echo "Starting backup (this may take a while)..."
echo "Backing up /mnt/nvme/* to $BACKUP_DEST/"
sshpass -p "$PI_PASSWORD" ssh -o StrictHostKeyChecking=no raolivei@$HOST "sudo rsync -avh --progress /mnt/nvme/ $BACKUP_DEST/ 2>&1" | tee /tmp/backup-progress.log

# Verify backup
echo ""
echo "Verifying backup..."
BACKUP_SIZE=$(sshpass -p "$PI_PASSWORD" ssh -o StrictHostKeyChecking=no raolivei@$HOST "sudo du -sh $BACKUP_DEST 2>/dev/null | awk '{print \$1}'" 2>&1)
