...
- **OpenClaw image vision support (llama3.2-vision:11b)** — added `llama3.2-vision:11b` with `"input": ["image", "text"]` to both `ollama-lan` and `ollama-tailscale` provider model lists. OpenClaw auto-discovers image-capable models by scanning `models.providers[id].models[].input` — no separate `tools.media` config needed. When a Telegram user sends a photo, OpenClaw routes it to `llama3.2-vision:11b` on the Mac, injects the description into the agent context, then continues with qwen3.6:35b-mlx for the actual reply. Chosen over qwen2-vl:7b for the extra parameter count (11B vs 7B, +3.5 GB) — better at UI screenshots and complex image reasoning; both fit alongside qwen3.6:35b-mlx on 48 GB unified memory. Mac prerequisite: `ollama pull llama3.2-vision:11b` (~7.9 GB).
