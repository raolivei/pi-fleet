...
- **OpenClaw overflowing on brand-new sessions (`reserveTokensFloor`/`contextTokens` miscalibrated)** — even a fresh `/new` session with zero conversation hit "Auto-compaction could not recover this turn." Root cause: an earlier fix bumped `reserveTokensFloor` 20000→24000 without checking it against the real providers. `safeguard` mode computes usable budget as `contextWindow - reserveTokensFloor`; against `qwen2.5:32b`'s 32768 window that left only 8768 tokens, while the baseline system prompt (tools + workspace context, no conversation) measured ~9748 tokens on its own — confirmed live via `context-overflow-precheck` logs on a session with `messages=0`. Set `reserveTokensFloor: 8192` (matches `qwen2.5:32b`'s `maxTokens`) and global `contextTokens: 24000` (was `80000`, tuned for gemma4's 131072 window, never adjusted after the primary swap to qwen2.5:32b). Also raised `ollama-cluster/qwen2.5:3b`'s declared `contextWindow` 16384→32768 (+ matching `OLLAMA_CONTEXT_LENGTH` in `ollama-fallback.yaml`, + memory request/limit 2Gi/4Gi→2.5Gi/5Gi for the larger KV cache) — at the old 16384 the cluster fallback was structurally unable to hold the baseline prompt at all, independent of the reserve bug.
