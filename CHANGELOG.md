...
- **`ansible/playbooks/configure-persistent-journal.yml`** — standalone, idempotent playbook to make the systemd journal survive reboots, which it was silently not doing. Root cause: Raspberry Pi OS ships `/usr/lib/systemd/journald.conf.d/40-rpi-volatile-storage.conf`, which forces `Storage=volatile` and — because later-loaded drop-ins win — overrides `Storage=persistent` in `/etc/systemd/journald.conf` regardless of what that file says. It exists to spare SD cards from write wear; these nodes boot from NVMe. `/var/log/journal` also did not exist on any of the three nodes, which independently prevents `Storage=persistent` from taking effect even without the drop-in. The previous approach (a couple of `lineinfile` tasks inside `setup-hardware-watchdog.yml`) set the config but never verified it actually took — this playbook does, via `journalctl --header`, and fails loudly if the active journal file isn't under `/var/log/journal`. Applied live to node-2 and node-3; **note journald persistence protects against reboots and clean shutdowns but not an abrupt power cut** — see `docs/POE-POWER-INCIDENT-2026-09-15.md`.

- **`docs/POE-POWER-INCIDENT-2026-09-15.md`** — incident writeup for a multi-hour window where all three nodes independently went unreachable, most likely from the shared PoE+ switch (TP-Link TL-SG1008MP, 126W/30W-per-port budget) undersupplying one or more Raspberry Pi 5 8GB nodes under load. Documents the evidence (an ICMP timing pattern on node-3 consistent with a stalled CPU releasing a queued reply backlog, and a `sudo: Input/output error` on node-1 indicating a live filesystem fault rather than a network timeout), why journald's existing persistent-storage config didn't help, and follow-up recommendations (verify node-1 filesystem integrity, confirm PoE HAT model/actual delivery under load, consider a dedicated supply). Hardware watchdog was disabled cluster-wide pending resolution, since it was adding uncontrolled reboots on top of the power problem rather than protecting against a genuine software hang.
- **kube-vip HA VIP (`10.0.0.100`)** — the cluster referenced a kube-vip VIP (`kube_vip_ip: 192.168.2.100` in `group_vars/all.yml`, comments in `setup-cluster.yml`, `core-infrastructure/kustomization.yaml` claiming "MetalLB removed - kube-vip now handles LoadBalancer services") but **nothing actually deployed it** — no Ansible task, no Flux resource, no workload in the cluster. `kubectl` and every kubeconfig pointed at a single node's IP, so a hang on that one node (as happened with node-2 on 2026-09-15) takes down cluster access even though etcd survives. Added `core-infrastructure/kube-vip/` (RBAC + DaemonSet, ARP mode, control-plane nodeSelector) and wired it into the `core-infrastructure` kustomization. VIP is `10.0.0.100` on the **management** network, not the previously-referenced `192.168.2.100` on gigabit — the gigabit segment is only reachable from the Mac when the USB ethernet adapter is plugged in, so a gigabit-only VIP would defeat the purpose. `svc_enable=true` also gives it the LoadBalancer-provisioning role the old comment implied, for services (`traefik`, `bind9`) that have sat with `<pending>` external IPs.

- **`ansible/playbooks/configure-node-network.yml`** — the missing playbook that makes node network config recoverable from Ansible. Applies the canonical post-migration layout on every node: `eth0` = gigabit/k3s (`node_N_ip`, **no gateway**, `ipv4.never-default=yes`) and `wlan0`/homebase = management/WAN (`node_N_mgmt_ip`, gw `management_gateway`, DNS `management_dns`). Discovers NM profile names at runtime rather than hardcoding them, reactivates `wlan0` fire-and-forget + `wait_for_connection` so the play survives its own control-channel drop, and asserts egress does **not** leave via `192.168.2.x`. Until now `node_N_mgmt_ip`, `management_gateway`, and `management_dns` existed in `group_vars/all.yml` but **no playbook consumed them** — a node's network could not be restored by re-running Ansible.
...
- **Public ingress had no real protection against a node-1 outage** — `traefik` and `cloudflared` both ran 2 replicas but with only `preferredDuringSchedulingIgnoredDuringExecution` (soft) pod anti-affinity, so the scheduler could legally co-locate both on the same node, and neither deployment excluded node-1 (labelled `eldertree.xyz/node-tier=unstable` by the existing `node-scheduling-tier-reconciler`, which node-1's recurring hangs already justified). `personal-website` ran a single replica with no anti-affinity at all — any node hosting it was a hard single point of failure. Brought all three in line with the pattern `swimto`'s `helmrelease.yaml` already used and documented (issue #228): `requiredDuringSchedulingIgnoredDuringExecution` pod anti-affinity (hostname topology) plus `nodeAffinity` excluding `node-tier: unstable`, with `maxSurge: 0` / `maxUnavailable: 1` on the rollout strategy — required because with only 2 legal nodes for 2 replicas under hard anti-affinity, the chart/deployment defaults (`maxSurge: 1`) would try to schedule a 3rd pod that has no legal home and stall the rollout.
- **`swimto-api`/`swimto-web` already had the correct affinity in git, but the live cluster had drifted from it** — a pod already running on node-1 predates `requiredDuringSchedulingIgnoredDuringExecution` only being enforced at scheduling time, not continuously, so Kubernetes never evicted it after the rule was added or after node-1 was (re)labelled `unstable`. `swimto-web` was also live at `replicas: 1` against `replicas: 2` in git. A rollout restart + scale was needed to actually apply the already-correct configuration.

- **CoreDNS held two stale ClusterIPs, breaking every `*.eldertree.local` name and starving external-dns** — `coredns-custom` mapped all `*.eldertree.local` hosts to `192.168.2.200` (an address no longer assigned to anything; Traefik's LoadBalancer is now the node IPs) and forwarded the `fallthrough` to `10.43.127.139` for BIND9, whose ClusterIP is actually `10.43.237.251`. Every lookup therefore timed out, flooding CoreDNS with `i/o timeout` errors. That flood also starved ordinary cluster-internal resolution, so `external-dns` crash-looped on `lookup bind9.bind.svc.cluster.local: i/o timeout` and could never write records — leaving the BIND9 zone frozen at serial `2026060901` (June 9). Corrected both addresses; added a comment recording that these are ClusterIPs and the two `kubectl` commands to re-derive them, since a Service recreation silently breaks cluster DNS again. After the fix `vault.eldertree.local` resolves in-cluster and external-dns writes the full record set.

- **ESO pointed at the round-robin `vault` Service, so secret sync broke whenever a pod restarted** — `ClusterSecretStore/vault` used `http://vault.vault.svc.cluster.local:8200`, which load-balances across every Vault pod. A sealed or non-member standby answers a read with a 307 redirect to the *leader's pod IP*; ESO follows it, and the moment that pod is recreated the cached address is dead — observed as `dial tcp 10.42.1.71:8200: connect: no route to host` across 24 ExternalSecrets after a `vault-0` restart, plus intermittent `Vault is sealed` store-validation failures. Repointed at `vault-active`, which selects only the current leader via Vault's Kubernetes service registration. 29/30 ExternalSecrets sync after the change.

- **`configure-k3s-gigabit.yml` wrote an unreachable `node-ip`, breaking k3s startup** — the `node_config` map had `node_ip` sourced from `node_N_eth0_ip` (the **management** address) and `wifi_ip` from `node_N_ip` (the **gigabit** address): exactly inverted. node-1's generated `/etc/rancher/k3s/config.yaml` carried `node-ip: 172.16.0.1` / `advertise-address: 172.16.0.1` from an even older revision of those vars, matching no interface, so k3s died on every start with `failed to start networking: unable to initialize network policy controller: error getting node subnet: failed to find interface with specified node ip` and restart-looped. Renamed `wifi_ip` → `mgmt_ip`, pointed `node_ip` at `node_N_ip` (gigabit/k3s) and `mgmt_ip` at `node_N_mgmt_ip`, and changed the join URL from `https://{{ node_1_eth0_ip }}:6443` to `https://{{ kube_vip_ip }}:6443` so joining nodes target the HA VIP rather than one node's management address.

- **Stale node addressing across docs, scripts, and tables** — `scripts/verify-watchdog.sh` held `NODE_IPS_GB=("10.0.0.1" "10.0.0.2" "10.0.0.3")`: wrong octets *and* mislabeled (10.0.0.x is the management network; `10.0.0.1` is the home router). Renamed to `NODE_IPS_MGMT` (`10.0.0.10x`) / `NODE_IPS_GB` (`192.168.2.10x`) and made SSH try management first, since the gigabit segment is not routable from the Mac. The node tables in `CLAUDE.md`, `README.md`, `docs/NETWORK.md`, `docs/NETWORK_ARCHITECTURE.md`, and `clusters/eldertree/openclaw/docs/ELDERTREE_KNOWLEDGE.md` all had the two columns **swapped**; corrected, with an explicit note that `10.0.0.1` is the router. `docs/HARDWARE_WATCHDOG.md` SSH examples retargeted to `10.0.0.101`. `docs/NODE_TROUBLESHOOTING*.md` no longer assert the old layout as "✅ Correct". Historical incident reports (`NODE-1-HANG-*`, old setup guides) were left as-is so the record stays accurate.

- **node-1 network: interface IPs were swapped, colliding with the home router** — node-1 had `wlan0`(homebase) = `192.168.2.101/24` gw `192.168.2.1` and `eth0` = `10.0.0.1/24`, i.e. the exact inverse of node-2/node-3. Two consequences: (1) `10.0.0.1` is the **home router's own IP**, so every packet the Mac sent to "node-1" was answered by the router — SSH appeared to time out while ICMP succeeded; (2) the default route pointed at `192.168.2.1`, which does not exist (the gigabit switch has no gateway), so node-1 had **no internet**, could not reach the cluster, failed the watchdog's k3s health check, and rebooted in a loop. Corrected to the canonical layout (`eth0`=`192.168.2.101` no gw, `wlan0`=`10.0.0.101` gw `10.0.0.1`). All three nodes now share one layout — node-1 is **no longer "inverted"**, and the stale comments saying so in `group_vars/all.yml` were removed.

- **`ansible/playbooks/fix-network-config.yml` deprecated — it *causes* the failure above** — the playbook still encodes the pre-migration layout: it writes the `192.168.2.x` k3s address onto `wlan0` and sets `ipv4.gateway 192.168.2.1` on it, which blackholes all egress. This is precisely the state node-1 was found in. Added a fail-fast guard (overridable only via `-e i_understand_this_breaks_networking=true`) plus a header pointing at `configure-node-network.yml`.
