...
- **OpenClaw image vision support (minicpm-v:8b)** — added `minicpm-v:8b` with `"input": ["image", "text"]` to both `ollama-lan` and `ollama-tailscale` provider model lists. OpenClaw auto-discovers image-capable models by scanning `models.providers[id].models[].input` — no separate `tools.media` config needed. When a Telegram user sends a photo, OpenClaw routes it to `minicpm-v:8b` on the Mac, injects the description into the agent context, then continues with qwen3.6:35b-mlx for the actual reply. `llama3.2-vision:11b` was tried first but the `mllama` architecture fails to load on this Ollama/Apple Silicon setup; `minicpm-v:8b` is purpose-built for screenshot and UI understanding and outperforms larger vision models on those tasks. Mac prerequisite: `ollama pull minicpm-v:8b` (~5 GB, already pulled).
