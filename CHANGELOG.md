...
- **OpenClaw OpenRouter 402 / prompt over limit + compaction failures** — OpenRouter rejected ~38k prompt tokens against a ~25.6k cap for this route. In [`configmap.yaml`](clusters/eldertree/openclaw/configmap.yaml): set catalog `contextWindow` for `google/gemini-2.0-flash-001` to **24000** (was 1M), `maxTokens` **3072** for listed models; add `agents.defaults.contextTokens` **22000**, `compaction.mode` **safeguard** with `compaction.model` **Groq** (offloads summarization from OpenRouter credits), `compaction.memoryFlush.enabled` **false** to skip an extra pre-compaction turn when balance is tight. (Omit `agents.defaults.params` — not accepted on gateway v2026.3.28.) Re-add OpenRouter credits or raise limits if you need longer sessions.
- **OpenClaw `trusted_proxy_untrusted_source` / internal WebSocket `1008 unauthorized`** — Trusted-proxy auth cannot satisfy loopback gateway clients (they do not send `X-Forwarded-User`; see [openclaw#43300](https://github.com/openclaw/openclaw/issues/43300)). Switched [`configmap.yaml`](clusters/eldertree/openclaw/configmap.yaml) to `gateway.auth.mode: "token"` with `token: "${OPENCLAW_GATEWAY_TOKEN}"`. Removed Traefik `add-trusted-proxy-user` middleware from [`helmrelease.yaml`](clusters/eldertree/openclaw/helmrelease.yaml). **Web UI:** paste gateway token once (Vault `secret/openclaw/gateway` / `openclaw-secrets` `OPENCLAW_GATEWAY_TOKEN`). Documented in [`README.md`](clusters/eldertree/openclaw/README.md) and [`skills-configmap.yaml`](clusters/eldertree/openclaw/skills-configmap.yaml) (no `openclaw update` in container — use GHA + Flux).
