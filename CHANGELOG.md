...
- **Observability (Eldertree)** — Traefik metrics exposed on the k3s Traefik `Service` and scraped via static `additionalScrapeConfigs` (`traefik-scrape-config`); SwimTO `swimto-api` Service annotated for Prometheus app `/metrics`; blackbox exporter + HTTPS probes + `BlackboxProbeFailing` alert; Loki 2.9 + Promtail + Grafana Loki datasource; **Eldertree Ops Home** dashboard (`eldertree-ops-home.json`). See `helm/monitoring-stack/DASHBOARDS.md` (Traefik v3 label source of truth).
