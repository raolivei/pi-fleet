...
- **OpenClaw image vision support (qwen2-vl:7b)** — added `qwen2-vl:7b` with `"input": ["image", "text"]` to both `ollama-lan` and `ollama-tailscale` provider model lists. OpenClaw auto-discovers image-capable models by scanning `models.providers[id].models[].input` — no separate `tools.media` config needed. When a Telegram user sends a photo, OpenClaw routes it to `qwen2-vl:7b` on the Mac, injects the description into the agent context, then continues with qwen3.6:35b-mlx for the actual reply. Mac prerequisite: `ollama pull qwen2-vl:7b` (~4.4 GB, fits comfortably alongside qwen3.6:35b-mlx's 21 GB on 48 GB unified memory).
