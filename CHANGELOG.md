...
- **OpenClaw OpenRouter 402 / prompt over limit + compaction failures** — OpenRouter rejected ~38k prompt tokens against a ~25.6k cap for this route. In [`configmap.yaml`](clusters/eldertree/openclaw/configmap.yaml): set catalog `contextWindow` for `google/gemini-2.0-flash-001` to **24000** (was 1M), `maxTokens` **3072** for listed models; add `agents.defaults.params.maxTokens` 3072, `contextTokens` 22000, `compaction.mode` **safeguard** with `compaction.model` **Groq** (offloads summarization from OpenRouter credits), `compaction.memoryFlush.enabled` **false** to skip an extra pre-compaction turn when balance is tight. Re-add OpenRouter credits or raise limits if you need longer sessions.
