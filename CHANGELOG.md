...
- **Hardware Watchdog Monitoring** — BCM2835 watchdog enables automatic node recovery from system freezes (#153). Ansible playbook `setup-hardware-watchdog.yml` installs watchdog daemon with 15s timeout, monitors k3s via pidfile, and pings cluster nodes on gigabit network (10.0.0.0/24). New documentation: `docs/HARDWARE_WATCHDOG.md` with deployment, verification, troubleshooting, and threshold tuning. Addresses Node-1 freeze (Feb 13-17) where kubelet stopped responding but system remained pingable.
